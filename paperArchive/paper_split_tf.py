...
split_label = ["0", "1"]
sys_train_file = "paper_train.xlsx"
sys_eval_file = "paper_test.xlsx"

class PaperSplitProcessor(DataProcessor):
    ...
    def get_excel_examples(self, path):
        examples_eval = []
        labels = self._read_excel(
            os.path.join(path,
                         sys_eval_file), 1)
    ...
    def get_ab_excel_examples(self, ab_path):
        examples_eval = []
        labels = self._read_excel(ab_path, 1)
        texts = self._read_excel(ab_path, 0)
        for (i, label) in enumerate(labels):
            if i == 0:
                continue
            guid = "dev-%d" % (i)
...
class PaperSplitEngine(object):
    """Wapper of the the paper split classification engine.
    """
    def __init__(self):
        self.FLAGS = FLAGS
        self.data_dir = './data/paper_split/'
        self.task_name = 'paper_split'
        self.vocab_file = 'multi_cased_L-12_H-768_A-12/vocab.txt'
        self.bert_config_file = 'multi_cased_L-12_H-768_A-12/bert_config.json'
        self.output_dir = './result/paper_split_base_v1.0/'
        ...
        # set train/eval file
        # if self.do_train:
        #     self.FLAGS.train_file = "paper_train.xlsx"
        # if self.do_eval:
        #     self.FLAGS.eval_file = "test_new_split_210114.xlsx"

        tf.logging.set_verbosity(tf.logging.INFO)

        processors = {
            'paper_split': PaperSplitProcessor
        }
    ...
    def model_eval(self):
        # eval_examples = self.processor.get_dev_examples("/home/jhgao/bert/")
        eval_examples = self.processor.get_excel_examples(path="/home/jhgao/bert/")
    ...
    def model_eval_excel(self, path):
        eval_examples = self.processor.get_ab_excel_examples(path)
        num_actual_eval_examples = len(eval_examples)
        if self.FLAGS.use_tpu:
            # TPU requires a fixed batch size for all batches, therefore the number
            # of examples must be a multiple of the batch size, or else examples
        ...
        with tf.gfile.GFile(output_eval_file, "w") as writer:
            ...
            for key in sorted(result.keys()):
                tf.logging.info("  %s = %s", key, str(result[key]))
                writer.write("%s = %s\n" % (key, str(result[key])))
        return result["eval_accuracy"], result["recall"]


    def model_eval_word(self, texts):
        eval_examples = self.processor.get_batch_examples(texts)
        num_actual_eval_examples = len(eval_examples)
        if self.FLAGS.use_tpu:
            # TPU requires a fixed batch size for all batches, therefore the number
            # of examples must be a multiple of the batch size, or else examples
    ...
    def fast_demo(self, texts):
        ...
        for (i, result) in enumerate(results):
            ...
            # print(prob)
            predicts.append(split_label[prob.index(max(prob))])
